...
namespace KoraEditor.UI
{
    internal sealed class ImGuiContext
    {
        ...
        // Methods
        public async void Initialize(GraphicsProvider graphics, AssetProvider assets)
        {
            ...
            unsafe
            {
                ...
                graphics.UploadTexture(fontTexture);
        ...
        {
            ImGuiIOPtr io = ImGui.GetIO();
            io.DisplaySize = new Vector2(graphics.RenderWidth, graphics.RenderHeight);
            io.DeltaTime = 1f / 60f;
        ...
        public void EndFrame()
        {
            ...
            RenderDrawData(drawPtr);

            // Submit the command buffer
            graphics.Submit();
        }

        private unsafe void RenderDrawData(ImDrawDataPtr drawPtr)
        {
            ...
            graphics.BeginCopyPass();
            {
                graphics.UploadBuffer(vertexBuffer);
                graphics.UploadBuffer(indexBuffer);
            }
            graphics.EndCopyPass();

            // Begin render pass now that buffers are on the GPU
            graphics.BeginRenderPass(Color.CornflowerBlue);
            {
                // Set the viewport
                unsafe
                {
                    ...
                    SDL3.SDL_SetGPUViewport(graphics.gpuRenderPass, &viewport);
                }

                // Bind buffers and pipeline
                graphics.BindVertexBuffer(vertexBuffer);
                graphics.BindIndexBuffer(indexBuffer, IndexBufferFormat.Int16);
                material.Bind(graphics, MeshVertexElements.Position2 | MeshVertexElements.UV | MeshVertexElements.Color32);
                ...
                graphics.BindUniform(mvp, 0, ShaderStage.Vertex);
                ...
                for (int i = 0; i < drawPtr.CmdListsCount; i++)
                {
                    ...
                    for (int j = 0; j < listPtr.CmdBuffer.Size; j++)
                    {
                        ...
                        SDL3.SDL_SetGPUScissor(graphics.gpuRenderPass, &clipRect);
                        ...
                        if (cmdPtr.TextureId != IntPtr.Zero)
                        {
                            ...
                            {
                                graphics.BindTexture(fontTexture, 0);
                            }
                        }

                        // Draw
                        SDL3.SDL_DrawGPUIndexedPrimitives(graphics.gpuRenderPass, cmdPtr.ElemCount, 1, (uint)(cmdPtr.IdxOffset + indexOffset), (int)(cmdPtr.VtxOffset + vertexOffset), 0);
                    }
