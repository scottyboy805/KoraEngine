...
namespace KoraGame
{
    ...
    internal sealed class GameApp : Game
    {
        ...
        internal override void DoInitialize()
        {
            ...
            // Create graphics            
            this.graphics = new GraphicsProvider(this.screen);

            Debug.Log($"Use graphics API: '{graphics.GetDeviceDriverName()}'", LogFilter.Graphics);

            // Create assets
            Debug.Log($"Initialize assets", LogFilter.Assets);
            this.assets = new AssetProvider(scriptable, graphics, Environment.CurrentDirectory + "/Assets", false);
            ...
            GameObject physicsCube = assets.LoadAsync<GameObject>("DefaultAssets/Cube.fbx").Result; //GameObject.PrimitiveCube(graphics, new Vector3F(2f));
            ...
            cam.GetComponent<Camera>().ClearColor = Color.CornflowerBlue;
            cam.SetActive(true);
            ChangeScene(scene);
        }
